...
void GridWorld::displayPath(vertex* originVertex)
{
	...
	while(1)
	{
		min_g_plus_c = INF;
		for(int m=0; m < DIRECTIONS; m++){
			vertex* neighbour = currentVertex->move[m];
	...
	if (currentVertex == NULL || currentVertex == &map[goalVertex.row][goalVertex.col])
	{
		break;
...
void GridWorld::displayTheLPAStarPath()
{	
 ...
 while(1){
 	...
 	if(min_neighbour != NULL)
	{
     ...
     if(min_neighbour == NULL || min_neighbour == &map[startVertex.row][startVertex.col])
