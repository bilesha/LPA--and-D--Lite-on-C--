...
public:
	...
	void displayPath(vertex* originVertex);
	void displayTheLPAStarPath();
   //------------------------------------------------
	void drawGrid();
   ...
   int getFieldX1(){
	...
	void setStartVertex(vertex s){
		...
		if( (s.row != -1) && (s.col != -1) )
		{
		   setMapTypeValue(s.row, s.col, '6');
		   startVertex = map[s.row][s.col];	
		   //startVertex.type = '6';
		}	
      else{ 
	...
	void setGoalVertex(vertex s){
		...
		if( (s.row != -1) && (s.col != -1) )
		{
		   setMapTypeValue(s.row, s.col, '7');
		   goalVertex = map[s.row][s.col];	
		}
	   else{ 
